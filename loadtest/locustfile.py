...
class MalaMaturaTaskSet(TaskSequence):
    ...
    def zapocni_test(self):
        r = self.client.post('/api/pocetak/', json={'test': self.test_id}, headers=self.headers)
    ...
    def posalji_odgovor(self):
        ...
        r = self.client.post('/api/odgovor/', json=odg, headers=self.headers)
    ...
    def post_zavrsi(self):
        self.client.post('/api/kraj/', json={'test_ucenika': self.test_ucenika_id}, headers=self.headers)
        raise StopLocust()


class Ucenik(HttpLocust):
